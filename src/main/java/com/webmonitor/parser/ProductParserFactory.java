...
/**
 * 제품 URL에 따라 적절한 파서를 반환하는 Factory
 */
@Component
...
public class ProductParserFactory {
     ...
     */
    public ProductParser getParser(String url) {
        try {
            URI uri = new URI(url);
            ...
            for (ProductParser parser : parsers) {
                ...
                if (host.equals(supportedDomain) || host.endsWith("." + supportedDomain)) {
                    log.debug("제품 파서 찾음: {} -> {}", url, parser.getClass().getSimpleName());
                    return parser;
                }
            }

            log.debug("지원되지 않는 도메인: {} (사용 가능한 제품 파서: {})", host, parsers.size());
            return null;

        } catch (URISyntaxException e) {
            log.error("제품 URL 파싱 중 오류: {}", url, e);
            return null;
        }
    }
